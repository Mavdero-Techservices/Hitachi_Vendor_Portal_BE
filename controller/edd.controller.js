...
exports.postEddDetails = async (req, res) => {
  ...
  for (let i = 0; i < req.body.length; i++) {
    InvoiceSchema.findOne({ where: { key: req.body[i].key, } }).then((eddDetails) => {
      if (eddDetails) {
        req.body[i].Document_Type = "Invoice";

        InvoiceSchema.update(req.body[i], {
          where: {
            key: req.body[i].key,
          },
        })
        ...
        const user = new InvoiceSchema({
          ...
          Document_No: req.body[i].Document_No,
          Line_No: req.body[i].Line_No,
          glCode: req.body[i].Type,
          No: req.body[i].No,
          Location_Code: req.body[i].Location_Code,
          Unit_of_Measure: req.body[i].Unit_of_Measure,
          qty: req.body[i].Quantity,
          baseAmount: req.body[i].Amount,
          Maintenance_Code: req.body[i].Maintenance_Code,
          Expected_Receipt_Date: req.body[i].Expected_Receipt_Date,
          EDD_Type: req.body[i].EDD_Type,
          End_Date: req.body[i].End_Date,
          Start_Date: req.body[i].Start_Date,
          Shortcut_Dimension_2_Code: req.body[i].Shortcut_Dimension_2_Code,
          Shortcut_Dimension_1_Code: req.body[i].Shortcut_Dimension_1_Code,
          Quantity_Invoiced: req.body[i].Quantity_Invoiced,
          Quantity_Received: req.body[i].Quantity_Received,
          Qty_to_Receive: req.body[i].Qty_to_Receive,
          Qty_to_Invoice: req.body[i].Qty_to_Invoice,
          Outstanding_Quantity: req.body[i].Outstanding_Quantity,
          Description: req.body[i].Description,
          Description_2: req.body[i].Description_2,
          ETag: req.body[i].ETag,
          key: req.body[i].key,
          vendorName: req.body[i].vendorName,
        });
